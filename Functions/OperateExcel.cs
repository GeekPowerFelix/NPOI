        ...
        public class ExcelHelper : IDisposable
        {
            ...
            public static int DataTableToExcel(string TargetFileNamePath, DataTable sourceData, string sheetName, bool IsWriteColumnName)
            {
                ...
                if (IsWriteColumnName)
                {
                    ...
                    {
                        //create null column and det value from sourceData
                ...
                //写入数据
                for (int row = 0; row < sourceData.Rows.Count; row++)//rows
                {
                    //sheet表创建新的一行
                    IRow newRow = sheet.CreateRow(WriteRowCount);   //columns
                    for (int column = 0; column < sourceData.Columns.Count; column++)
                    {
                        if (column > 7)
                        {
                            newRow.CreateCell(column).SetCellValue(int.Parse(sourceData.Rows[row][column].ToString()));
